...
public class aimAndShootCommand extends Command {
  ...
  private final ProfiledPIDController thetaController =
      new ProfiledPIDController(
          ...
          new TrapezoidProfile.Constraints(
              Units.degreesToRadians(360.0), 5));//TODO
  ...
  public void initialize() {
    ...
    thetaController.reset(
        ...
        ChassisSpeeds.fromFieldRelativeSpeeds(drive.getChassisSpeeds(),currentPose.getRotation()).omegaRadiansPerSecond);
  ...
  public void execute() {
    ...
    double thetaFFScaler =
        MathUtil.clamp(
            (Units.radiansToDegrees(thetaErrorAbs)/Math.PI),
            0.0,
            1.0);//计算动态前馈参数
    ...
    //======command drive=======//
    drive.runVelocity(
        new ChassisSpeeds(0,0,thetaVelocity)
    );

    //=======log states=======//
    Logger.recordOutput("aimAndShootCommad/ThetaMeasured",currentPose.getRotation());
    Logger.recordOutput("aimAndShootCommad/ThetaSetpoint",Rotation2d.fromRadians(thetaController.getSetpoint().position));
